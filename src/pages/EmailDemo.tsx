...
const EmailDemo: React.FC = () => {
  ...
  const [emailResults, setEmailResults] = useState<Array<{
    type: string;
    success: boolean;
    message: string;
    timestamp: Date;
  ...
  const sendWelcomeEmail = async () => {
    ...
    try {
      ...
      addResult("Welcome Email", success, success ? "Email sent successfully!" : "Email service not configured");
    } catch (error) {
      addResult("Welcome Email", false, `Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const sendTestOrderConfirmation = async () => {
    ...
    try {
      const mockOrderData: OrderConfirmationEmailData = {
        ...
        items: [
          { name: "Vintage Vinyl Record - The Beatles", quantity: 1, price: 24.99 },
          { name: "Retro Sunglasses", quantity: 2, price: 15.50 },
          { name: "Classic Band T-Shirt", quantity: 1, price: 19.99 }
        ],
        total: 75.98,
        shippingAddress: "123 Nostalgia Lane\nVintage City, VC 12345\nUnited States"
      };

      const success = await emailService.sendOrderConfirmationEmail(authState.user.email, mockOrderData);
      addResult("Order Confirmation", success, success ? "Email sent successfully!" : "Email service not configured");
    } catch (error) {
      addResult("Order Confirmation", false, `Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setSendingEmail(null);
    }
  };
  ...
  const sendCustomEmail = async () => {
    ...
    try {
      ...
      addResult("Custom Email", success, success ? "Email sent successfully!" : "Email service not configured");
    } catch (error) {
      addResult("Custom Email", false, `Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setSendingEmail(null);
    }
  };
  ...
  const loadDomains = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        addResult("Load Domains", true, `Found ${result.data.data?.length || 0} domains`);
      } else {
        addResult("Load Domains", false, result.error || "Failed to load domains");
      }
    } catch (error) {
      addResult("Load Domains", false, `Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const createDomain = async () => {
    ...
    try {
      ...
      if (result.success) {
        addResult("Create Domain", true, `Domain '${newDomainName}' created successfully!`);
        await loadDomains(); // Refresh the list
      } else {
        addResult("Create Domain", false, result.error || "Failed to create domain");
      }
    } catch (error) {
      addResult("Create Domain", false, `Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setDomainAction(null);
    }
  };
    ...
    <div className="page-container">
      <div className="max-w-4xl mx-auto">
        <div className="page-header text-center mb-12">
          <h1 className="text-4xl font-bold mb-4" style={{ color: "var(--rich-black)" }}>
            📧 Email Service Demo
          </h1>
        ...
        <div className={`p-6 rounded-lg mb-8 ${isConfigured ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-3 h-3 rounded-full ${isConfigured ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
            <h3 className="text-lg font-semibold">
              Email Service Status: {isConfigured ? 'Configured' : 'Not Configured'}
            </h3>
          </div>
          ...
          {isConfigured ? (
            ...
            <div className="text-yellow-700">
              <p className="mb-2">⚠️ Resend API key not found. To enable email functionality:</p>
              <ol className="list-decimal list-inside space-y-1 text-sm">
                <li>Sign up for a Resend account at <a href="https://resend.com" className="underline" target="_blank" rel="noopener noreferrer">resend.com</a></li>
                <li>Get your API key from the Resend dashboard</li>
                <li>Add <code className="bg-yellow-100 px-1 rounded">VITE_RESEND_API_KEY=your_api_key</code> to your .env file</li>
        ...
        <div className={`p-6 rounded-lg mb-8 ${authState.user ? 'bg-blue-50 border border-blue-200' : 'bg-red-50 border border-red-200'}`}>
          ...
          {authState.user ? (
            <div className="text-blue-700">
              ...
              {authState.user.profile?.firstName && (
                <p>👋 Name: {authState.user.profile.firstName} {authState.user.profile.lastName}</p>
              )}
            </div>
          ) : (
            <p className="text-red-700">
              ❌ Not logged in. Please <a href="#/login" className="underline">log in</a> to test email functionality.
            </p>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg border shadow-sm">
            <h3 className="text-lg font-semibold mb-3" style={{ color: "var(--denim-blue)" }}>
              Welcome Email
            </h3>
            <p className="text-sm text-gray-600 mb-4">
              Test the branded welcome email sent to new users after registration.
            </p>
            <button
        ...
        {isConfigured && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-6" style={{ color: "var(--rich-black)" }}>
              🌐 Domain Management
            </h2>

            {/* Create Domain */}
            <div className="bg-white p-6 rounded-lg border shadow-sm mb-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: "var(--denim-blue)" }}>
                Add Domain
              </h3>
              <div className="flex gap-4 items-end">
                ...
                <button
                  ...
                  disabled={domainAction === "creating" || !newDomainName.trim()}
                  className="btn-primary disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {domainAction === "creating" ? "Creating..." : "Create Domain"}
                </button>
              </div>
            ...
            <div className="bg-white rounded-lg border shadow-sm">
              <div className="p-6 border-b flex justify-between items-center">
                <h3 className="text-lg font-semibold" style={{ color: "var(--rich-black)" }}>
                  Your Domains
                </h3>
                <button
                  onClick={loadDomains}
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <div className="text-right">
                            ...
                            {domain.region && (
                              <div className="text-sm text-gray-500">Region: {domain.region}</div>
                            )}
                          </div>
                        </div>
                          ...
                          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
                            ...
                            <div className="space-y-2 text-xs">
                              {domain.records.map((record: any, index: number) => (
                                <div key={index} className="font-mono bg-white p-2 rounded">
        ...
        <div className="mt-12 bg-gray-50 p-6 rounded-lg">
          <h3 className="text-lg font-semibold mb-4" style={{ color: "var(--rich-black)" }}>
            📚 Integration Details
          </h3>
          <div className="space-y-3 text-sm">
            ...
            <div>
              <strong>Templates:</strong> HTML & text versions with branded styling
            </div>
            <div>
              ...
              <ul className="list-disc list-inside ml-4 mt-1 space-y-1">
                ...
                <li>Custom email sending (for notifications, marketing, etc.)</li>
              </ul>
            </div>
            <div>
            ...
            <div>
              <strong>Fallback:</strong> Graceful degradation when API key not configured
            </div>
          </div>
        </div>
      </div>
